   ...
   del_query = "DELETE * FROM [Racer-Score-U16] "
   wscript.echo del_query
   MyConn.Execute del_query

   SQL_query = "SELECT * FROM [racer-runs-u16]"
...
sub writescore
	...
	score_class = score_class + unused_score_class
rem	If runcount < minruns then
rem		score_class = 0
rem	end if
	If event_cnt < 2 then
		score_class = 0
	...
	ins_query = "INSERT INTO [Racer-Score-U16](ussa,name,class,gender,score_class,tie_score_class) "
