   ...
   del_query = "DELETE * FROM [Racer-Score-U14] "
   wscript.echo del_query
   MyConn.Execute del_query

   SQL_query = "SELECT * FROM [racer-runs-u14]"
   ...
   WHILE NOT RS.EOF
	...
	if RS("runplace") < 1000 and RS("racetype") = "sl" then
		runcount = runcount+1
		wscript.echo RS("ussa") & "-sl-" & runcount & "-" & RS("wcpoints")
		Select Case True
	...
	if RS("runplace") < 1000 and RS("racetype") = "sg" then
 		runcount = runcount+1
		wscript.echo RS("ussa") & "-sg-" & runcount & "-" & RS("wcpoints")
		Select Case True
		Case runcount < minruns 
   			score_class = score_class + RS("wcpoints") 
              		sg_switch = 1
	...
	if RS("runplace") < 1000 and RS("racetype") = "gs" then
		runcount = runcount+1
		wscript.echo RS("ussa") & "-gs-" & runcount & "-" & RS("wcpoints")
		Select Case True
		Case runcount < minruns 
   			score_class = score_class + RS("wcpoints") 
              		gs_switch = 1
...
sub writescore
 ...
 wscript.echo saveussa & "-eventcnt-" & event_cnt
	...
	ins_query = "INSERT INTO [Racer-Score-U14](ussa,name,class,gender,score_class,tie_score_class) "
